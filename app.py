import numpy as np
import pandas as pd
from flask import Flask, request, render_template

from sklearn.preprocessing import StandardScaler
from src.pipeline.predict_pipeline import CustomData, PredictPipeline 

#Always pass __name__ so Flask knows where to look for files
application = Flask(__name__) 
app = application

@app.route('/')
def index():
    return render_template('index.html')

@app.route('/predictdata', methods=['GET','POST'])
def predict_datapoint():
    
    if request.method == 'GET':
        return render_template('home.html')
    else:    
        # POST: Submitting data
        data = CustomData(
            gender=request.form.get('gender'),
            race_ethnicity=request.form.get('race_ethnicity'),
            parental_level_of_education=request.form.get('parental_level_of_education'),
            lunch=request.form.get('lunch'),
            test_preparation_course=request.form.get('test_preparation_course'),
            # The web sends "75" (text), the model needs 75.0 (number)
            reading_score=float(request.form.get('reading_score')),
            writing_score=float(request.form.get('writing_score')) 
        )
        
        data_df = data.get_data_as_data_frame()
        print(data_df) # Good for debugging

        predict_pipeline = PredictPipeline()
        
        results = predict_pipeline.predict(data_df)
        
        # We take results[0] because the model returns a list like [65.4]
        return render_template('home.html', results=results[0]) #label=value==> 1st results is label from html file and 2nd results = predict_pipeline.predict(data_df)

if __name__ == "__main__":
    app.run(host='0.0.0.0', debug=True)